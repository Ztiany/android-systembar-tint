...
public abstract class VerticalScrollingBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    private static final String TAG = VerticalScrollingBehavior.class.getSimpleName();

    private int mTotalDyUnconsumed = 0;//总的没有消费的Dy
    private int mTotalDy = 0;//总的Dy
    @ScrollDirection
   ...
   */
     ...
     * @param child 子View,自己
     * @param directTargetChild 直接目标Child
     * @param target 目标child
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, V child, View target, int dx, int dy, int[] consumed) {
        ...
        if (dy > 0 && mTotalDy < 0) {//dy > 0 说明现在是向上滑动，而mTotalDy<0说明刚刚是向下滑动，所以需要改变方向
            mTotalDy = 0;
            mScrollDirection = ScrollDirection.SCROLL_DIRECTION_UP;
        } else if (dy < 0 && mTotalDy > 0) {//dy < 0 说明现在是向下滑动，而mTotalD>0说明刚刚是向上滑动，所以需要改变方向
            mTotalDy = 0;
